...
export async function handleEntityUpdate(
  ...
  let parsedOutcomes = [];
  try {
    parsedOutcomes = JSON.parse(learningOutcomes || '[]')
    ...
    console.error('Failed to parse learning outcomes:', error);
    return res.status(400).json({ error: 'Invalid learning outcomes format' });
  }
