...
async function handleEntityUpdate(req, res, entityType) {
    ...
    // Handle file uploads
    if (files?.image) {
        updateData.imageUrl = await (0, supabase_1.uploadToSupabase)(files.image[0], `${entityType}s/images`);
    }
    if (files?.gif) {
        updateData.gifUrl = await (0, supabase_1.uploadToSupabase)(files.gif[0], `${entityType}s/gifs`);
    }
    // Handle extra images
    if (files?.extraImages) {
        const uploadPromises = files.extraImages.map(file => (0, supabase_1.uploadToSupabase)(file, `${entityType}s/extra-images`));
...
async function handleEntityCreate(req, res, entityType) {
    ...
    try {
        ...
        let extraImages = [];
        if (files?.image) {
            imageUrl = await (0, supabase_1.uploadToSupabase)(files.image[0], `${entityType}s/images`);
        }
        if (files?.gif) {
            gifUrl = await (0, supabase_1.uploadToSupabase)(files.gif[0], `${entityType}s/gifs`);
        }
        // Handle extra images
        if (files?.extraImages) {
            const uploadPromises = files.extraImages.map(file => (0, supabase_1.uploadToSupabase)(file, `${entityType}s/extra-images`));
...
async function handleReorder(req, res, entityType) {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const existingEntities = await tx[entityType].findMany({
            ...
            // Update positions
            await Promise.all(orderedIds.map((id, index) => tx[entityType].update({
                where: { id },
                data: { position: index }
            })));
            // Return the updated list
            return await tx[entityType].findMany({
                orderBy: { position: 'asc' },
                include: {
                    learningOutcomes: {
                        orderBy: { position: 'asc' }
